...
| 12 | 无法拖入新图片 | ✅ | 1.07 |
| 13 | 多选导入无法选择多个文件 | ✅ | 1.08 |
| 14 | 单文件关闭后未恢复初始状态 | ✅ | 1.08 |
| 15 | PNG格式导入导出支持 | ✅ | 1.09 |
...
| 功能 | 说明 |
|------|------|
| PNG 格式支持 | 支持导入和导出 PNG 文件 |
| PNG 透明通道 | 透明像素不参与颜色提取 |
| PNG EXIF 处理 | PNG 文件跳过 EXIF 读写 |
| 导出格式智能选择 | 根据原始文件类型自动选择导出格式 |

### 1.08 更新内容

| 功能 | 说明 |
|------|------|
| 多选导入 | 支持在文件对话框中按住Ctrl键选择多个JPG文件 |
| 关闭功能增强 | 单文件关闭后返回初始状态，多文件关闭后切换下一张 |
| UI标签优化 | 色号显示、名称显示、英文名称 |
...
| 多选导入 | 支持文件对话框多选和拖拽多文件 | ✅ |
| 单文件关闭 | 关闭单张图片后返回到初始状态 | ✅ |
